...
struct Graph* CreateGraph(int n)
{
	...
	vlist->vl = new nodeList[n+1];
 
	// Assign the head to NULL.
	for(i = 0; i < n+1; i++)
	{
		vlist->vl[i].listhead = NULL;
	}
...
void InsertNode(Graph *G, int v1, int v2, int weight)
{
	...
	if(G->vl[v2].listhead == NULL)
	{
		// If the head is null insert the node to the head.
		G->vl[v2].listhead = newnode1;
	}
	else
	{
		// Otherwise, add the node at the beginning.
		newnode1->link = G->vl[v2].listhead;
		G->vl[v2].listhead = newnode1;
	}
	// Connection 2, v1 to v2.
	if(G->vl[v1].listhead == NULL)
	{
		// If the head is null insert the node to the head.
		G->vl[v1].listhead = newnode2;
	}
	else if(v1 != v2)
	{
		// Otherwise, add the node at the beginning.
		newnode2->link = G->vl[v1].listhead;
		G->vl[v1].listhead = newnode2;
	}
}
